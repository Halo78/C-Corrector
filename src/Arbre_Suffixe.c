...
#include <string.h>
#include <ctype.h>
#include "../Include/Arbre_Suffixe.h"
#include <time.h>
#include <stdbool.h>
...
void redirection_prefix_suffixe(tree_t prefix, tree_t suffix) {

    tree_t prefix_2 = prefix->sons; 
    while (prefix_2 != NULL) { 
        tree_t suffix_2 = create_node(prefix_2->value); 
        suffix_2->brothers = suffix->sons; 
        suffix->sons = suffix_2; 
        redirection_prefix_suffixe(prefix_2, suffix_2); 
        prefix_2 = prefix_2->brothers; 
        
    }
}

tree_t compression_suffixe(tree_t prefix_tree) {
    tree_t suffix_tree = create_node('\0'); 
    redirection_prefix_suffixe(prefix_tree, suffix_tree); 
    return suffix_tree;
}
...
void compare(const char *dico_nom, const char *text_nom) {
  tree_t dico = create_node('\0');

  FILE *texte = fopen(text_nom, "r");
  FILE *intrus = fopen("Intrus.txt", "w");
  char word[256];


   ajouter_dico(dico, dico_nom) ; 
   dico = compression_suffixe(dico); 
...
void compare_analyse(tree_t dico,const char * dico_nom, const char * text_nom) {
